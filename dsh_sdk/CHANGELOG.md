...
- Add `DshPartitioner` enum incl. a topiclevel partitioner based on the murmur2 algorithm
  - `Stream` have now `compute_partition` method which uses the partitioner to compute the partition for a given key
  - Correct `DshPartitioner` can be returned aswel from a `Stream`
- Add feature flag `dsh-envelope` to have DSH Envelope structs and deserializer based on Prost

### Changed
- `read_access()` and `write_access()` are deprecated in favor of `is_readable()` and `is_writable()`, which provide clearer semantics.

### Fixed
- Return librdkafka compatible anchored read pattern in case of regex wildard from a `Stream`
- `verify_list_of_topics` now properly checks write topics including tenant name

## [0.8.1] - 2026-4-08
### Added
- HTTP Protocol Adapter client
- HTTP Protocol Base url to Platform enum 

### Changed
- Updated dependencies sha2
- Updated dev-dependencies RUMQTTC 
...
### Changed
- KAFKA_CONFIG_HOST changed to DSH_KAFKA_CONFIG_ENDPOINT due to deprecation of KAFKA_CONFIG_HOST in DSH

## [0.7.1] - 2025-07-18
### Changed
- Platform enum can handle custom defined platforms
- Plarform enum can be instantiated from environment variables
- The `ConfigGet` and `ConfigPut` structs of `schema_store` are now public, they were returned by public functions (required for compatibility reasons for future Rust versions)
...
- Updated dependencies
  - Updated rcgen to 0.14 (used in bootstrap)
  - Updated apache-avro to 0.19 (used in schema-store)
...
### Removed
- Hyper client client implementation (wasn't fully inplmenented)
